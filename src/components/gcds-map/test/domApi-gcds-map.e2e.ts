...
test.describe('gcds-map DOM API Tests', () => {
  let page: Page;
  ...
  test.beforeAll(async () => {
    ...
    await page.goto('/test/gcds-map/domApi-gcds-map.html', { waitUntil: 'networkidle' });
  });
  ...
  test('Create a gcds-map with document.createElement(gcds-map)', async () => {
    ...
    const velName = await page.evaluate(
      () => document.body.querySelector('gcds-map')?.nodeName
    );
  ...
  test('Create a layer with document.createElement(map-layer)', async () => {
    ...
    await page.evaluateHandle(
      (layer) => document.querySelector('gcds-map')?.appendChild(layer),
      layerHandle
    );
  ...
  test.skip('Remove gcds-map from DOM, add it back in', async () => {
    ...
    await viewer.evaluate(() => {
      let m = document.querySelector('gcds-map');
      if (m) {
  ...
  test('Create gcds-map without controls', async () => {
    // remove previously created map
    await page.evaluateHandle(() =>
      document.querySelector('gcds-map')?.remove()
    );
  ...
  test('Creating gcds-map with the default size hides the fullscreen control and the controls attribute functions properly', async () => {
    // Adding map
    const viewerHandle = await page.evaluateHandle(() =>
      document.createElement('gcds-map')
    );
  ...
  test.describe('controlslist test', () => {
    test('gcds-map created with controlslist', async () => {
      // Adding map
      const viewerHandle = await page.evaluateHandle(() =>
        document.createElement('gcds-map')
      );
